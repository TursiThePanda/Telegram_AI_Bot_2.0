...
async def handle_error(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if current_time - LAST_ERROR_REPORT_TIME < ERROR_REPORT_COOLDOWN:
        logger.warning(f"Error report skipped due to cooldown. Next report allowed in {ERROR_REPORT_COOLDOWN - (current_time - LAST_ERROR_REPORT_TIME):.1f}s.")
        return
    ...
    if isinstance(update, Update):
        try:
            ...
            # Remove potentially very long content like full photo data, etc.
    ...
    if len(message) > max_len:
        # Try to truncate gracefully, ensuring HTML tags are not cut
        # This is simplified; robust HTML truncation is complex.
        # For <pre> and <code>, which are mostly text-based, this is usually acceptable.
